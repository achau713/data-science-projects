...
p <- citations_dt %>% 
  ...
  geom_freqpoly(binwidth = 86400*7, color = "firebrick") +
...
# Number of transit lane violations by street name. 
...
# Citations by Year Visualization - Geary Boulevard
```{r}
...
p2 <- citations_streetName %>% 
  filter(street_name == "Geary") %>% 
  ggplot(aes(street_name)) +
  geom_bar(fill = "navy blue") +
  labs(x = "Geary Boulevard", y = "Citation Count", 
       title = "Citations count on Geary Boulevard from 2008-2015") + 
...
Pack year-by-year bar graph into a function
```{r}

streetName_barGraph <- function(street){
  barGraph <- citations_streetName %>% 
  filter(street_name == street) %>% 
  ggplot(aes(street_name)) +
  geom_bar(fill = "navy blue") +
  labs(x = street, y = "Citation Count", 
       title = cat("Citation count on", street, "from 2008-2015")) + 
  facet_grid(cols = vars(year)) +
  theme(axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(),
        axis.title.x = element_text(margin = margin(10, 0, 0, 0)), 
...
# Visulaize transit lane violations with Leaflet
```{r}
