...
# Convert citation date and citation dateTimeto date object to work in dplyr
muni$citation_date <- as.Date(muni$`Citation Issue Date`, format ="%m/%d/%Y")
...
muni <- muni %>% 
  select(-`Citation Issue Date`, -`Citation Issue Time`)
...
muni <- muni %>% 
  rename(ID = "Object ID", violation_type = "Violation")
...
muni <- muni %>% 
  select(ID, month, day, year, street_name, 
         violation_type, latitude, longitude, everything())
```


## sandbox - play around with data
```{r}
muni %>% 
...
muni$citation_date <- strptime(paste(muni$`Citation Issue Date`), format ='%m/%d/%Y')
