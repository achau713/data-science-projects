...
citationsTimeSeries <- citationsDateTime %>% 
  ggplot(aes(citation_dateTime)) +
...
# plotly plot
ggplotly(citationsTimeSeries, tooltip = c("y"))
...
```{r, fig.keep = 'last'}
citationsGroupedByStreet <- muni %>% 
  group_by(street_name, year) %>% 
  mutate(count = n())

citationsByStreet <- citationsGroupedByStreet %>% 
...
# glitchy
ggplotly(citationsByStreet, tooltip = "count")
...
citationsByType <- ggplot(data=muni, aes(x=violation_type)) + 
