...
muni$Citation.Date <- strptime(muni$Citation.Issue.Date, format ='%m/%d/%Y')


muni$Citation.FullDate <- strptime(paste(muni$Citation.Issue.Date, 
...
unique(muni$Street.Name)
unique(muni$Violation.Code)
unique(muni$Violation)
...
streetBarPlot <- ggplot(data=muni, aes(x=Street.Name)) + 
...
muni$Citation.Date <- as.Date(muni$Citation.Date)

# Group and count observations by date 
muni %>%
  group_by(Citation.Date) %>%
  count() %>%
  mutate(new=new)



timeSeries <- ts(muni$Citation.Date)
```
